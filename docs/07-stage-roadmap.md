...
etapa añade una capacidad sin reescribir las anteriores. **Hoy queda
completada la Etapa 4**: drag and drop manual.
...
| 5     | **Selection**        | 🔜             | Concepto de "objeto seleccionado". `SelectionBoxComponent`. Highlight visual + properties panel real. |
...
3. **El roadmap no es un commitment.** Es una guía. Si alguien aprendiendo
   ...
   permitirlo: cada etapa toca componentes distintos.
...
| 5 — Selection     | `selection-box/selection-box.{ts,html,css}`, signal `selectedObjectId`        | `BoardStateService` + nuevo método   |
